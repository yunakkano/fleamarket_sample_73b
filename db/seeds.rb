categories = [
  ...
  {level1: "メンズ", level1_children:[
    ...
    {level2: "バッグ", level2_children:[
      "ショルダーバッグ", "トートバッグ", "ボストンバッグ", "リュック/バックパック", "ウエストポーチ", "ボディバッグ", "ドラムバッグ", "ビジネスバッグ", "トラベルバッグ", "メッセンジャーバッグ", "エコバッグ", "その他"
      ]
    },
...
categories.each.with_index(1) do |category, i|
  ...
  eval("#{level1_var} = level1_val")                                                        #1階層の変数 = 1階層の値
    category[:level1_children].each.with_index(1) do |level1_child, j|
      level2_var = "#{level1_var}_#{j}"                                                     #2階層の変数 (ex.@category1_1)
      level2_val = eval("#{level1_var}.children.create(category: level1_child[:level2])")   #2階層の値作成 (ex.トップス)
      eval("#{level2_var} = level2_val")                                                    #2階層の変数 = 2階層の値
        level1_child[:level2_children].each do |level2_children_val|
          eval("#{level2_var}.children.create(category: level2_children_val)")              #3階層の値作成 (ex.@category1_1_1)
        end
    end
end
