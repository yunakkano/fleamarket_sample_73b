...
RSpec.describe SnsCredential do
  describe  '#create' do
    it 'uidに重複がなければ、user_idが空でもsns_credentialが登録できること' do
      user = build(:user)
    ...
    it 'uidが空場合にはsns_credentialが登録できないこと' do
      sns_credential = build(:sns_credential, uid: "")
      sns_credential.valid?
      expect(sns_credential.errors[:uid]).to include("が取得できませんでした")
    end

    it 'uidに重複が重複している場合には、providerが違っていてもsns_credentialが登録できないこと' do
